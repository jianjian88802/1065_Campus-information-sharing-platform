    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="资料介绍" prop="ziliaojieshao">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.ziliaojieshao" label="资料介绍" prop="ziliaojieshao">
                    <span v-html="ruleForm.ziliaojieshao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/ziyuanfenlei/fenlei`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
	...
	this.ruleForm.ziliaojieshao = this.ruleForm.ziliaojieshao.replace(reg,'../../../springboot2mc6m/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `ziliaofenxiang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.ziliaofenxiangCrossAddOrUpdateFlag = false;
                  this.parent.search();
